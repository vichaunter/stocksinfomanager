...
const URLS = {
    forMetrics: (symbol) => `https://seekingalpha.com/api/v3/metrics?filter[fields]=${metricFields.join("%2C")}&filter[slugs]=${symbol}&minified=false`,
    forRealTimeQuotes: (id) => `https://finance-api.seekingalpha.com/real_time_quotes?sa_ids=${id}`, //id comes from metrics
    forSymbolData: (symbol) => `https://seekingalpha.com/api/v3/symbol_data?fields[]=${dataFields.join("&fields[]=")}&slugs=${symbol}`, //here is dividend dates
};
// const keyMap = {
...
const fetchData = async ({ item }) => {
    ...
    for (let symbolUrl of symbolUrls) {
        const response = await fetch(symbolUrl(item.symbol)).then((res) => res.json());
...
const getMetric = (raw, id) => raw.forMetrics?.data?.find((m) => m.relationships?.metric_type?.data?.type === "metric_type" &&
    m.relationships?.metric_type?.data?.id === id);
const rawToTicker = (symbol, raw) => {
    let model = new tickerModel_1.default();
    model.symbol = symbol;
    const tickerRealTimeQuote = raw.forRealTimeQuotes?.real_time_quotes.find((q) => symbol === q.symbol);
    if (tickerRealTimeQuote) {
        model.price = tickerRealTimeQuote.last;
    }
    const dividendYield = getMetric(raw, "30");
    if (dividendYield)
        model.setDividendYield(dividendYield.attributes.value);
    ...
    if (dividend5YearGrowhthRate)
        model.setDividend5YearGrowhthRate(dividend5YearGrowhthRate.attributes.value);
    ...
    const dividendDates = raw.forSymbolData?.data?.[0]?.attributes?.dividends?.[0];
    if (dividendDates) {
